...
class Agent(object):
    ...
    def step(self):
        ...
        for dist in (sorted(mes_sensor_dist)):
            if dist < maxSensorDistance/2 and mes_sensors[mes_sensor_dist.index(dist)].layer=='joueur' and  agents[mes_sensors[mes_sensor_dist.index(dist)].sprite.numero].name==self.name:
                sensor_actif=mes_sensor_dist.index(dist)
                evite_joueur=True
                break
            elif dist < maxSensorDistance and mes_sensors[mes_sensor_dist.index(dist)].layer=='joueur' and agents[mes_sensors[mes_sensor_dist.index(dist)].sprite.numero].name != self.name:
                sensor_actif=mes_sensor_dist.index(dist)
                suis_joueur=True
                break
