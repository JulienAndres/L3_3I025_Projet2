...
class Agent(object):
    ...
    def step(self):
        ...
        if self.id==0 :
            ...
            for i in sorted(mes_sensor_dist):
                if mes_sensors[mes_sensor_dist.index(i)].layer=='joueur':
                    sensor_actif=mes_sensor_dist.index(i)
                    break
            
            if sensor_actif!=-1:
                if mes_sensor_dist[sensor_actif] < 15:
                    self.setRotationValue(SensorBelt[sensor_actif])
                    self.setTranslationValue(1)
                    return
                else:
