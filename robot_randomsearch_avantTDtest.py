...
nbAgents = 1 # doit être pair et inférieur a 32
...
showSensors = True
frameskip = 99999999999999999999999999999999999990   # 0: no-skip. >1: skip n-1 frames
verbose = True
...
class AgentTypeA(object):
    ...
    def stepSearchMethod(self): # random search
        if iteration==0:
            self.params=[random()*2-1,random()*2-1,random()*2-1,random()*2-1,random()*2-1,random()*2-1,random()*2-1,random()*2-1]
            self.bestParam=copy.deepcopy(self.params)
        if iteration % 1000 == 0:
            ...
            if iteration != 0:
                if self.bestFitness < self.fitness:
                    ...
                    self.bestParam=copy.deepcopy(self.params)
                    resetOccupancyGrid()
                    f=open("bestparam.txt","w")
                    s=str(self.fitness)+str(self.params)
                    f.write(s)
                print "Fitness:",self.fitness, "(best:", self.bestFitness,")",str(self.bestParam)
            print "Parameters:", str(self.params)

            print "Evaluation no.", int(iteration/1000)
            ...
            i=randint(0,len( SensorBelt)-1)
            self.params=copy.deepcopy(self.bestParam)
            self.params[i] += 0.05*gauss(0,1)
            if self.params[i]>1 or self.params[i]<-1 :
                self.params[i]=random()*2-1
